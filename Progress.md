...
- 已完成贡献任务：Day 1 到 Day 34
- 已完成并已正式提交到 GitHub：Day 1 到 Day 23
- 已完成但未正式提交到 GitHub：Day 24 到 Day 34（仅在本地 `contribution-queue` 队列中）
- 未完成且未提交到 GitHub：Day 35 到 Day 40
- 下一项任务：Day 35，`docs: add local troubleshooting FAQ`
...
| 34 | Done | `queue HEAD` | `test: cover prompt template loading` | 新增 Prompt 模板加载器单元测试，覆盖 classpath 加载缓存、变量渲染、section 渲染和缺失 section 报错。 |
...
```text
queue HEAD test: cover prompt template loading (queued)
...
当前 Day 24 到 Day 34 已在本地 `contribution-queue` 做成独立 commit，Day 35 到 Day 40 已完成规划。下一项任务是 Day 35：`docs: add local troubleshooting FAQ`。
