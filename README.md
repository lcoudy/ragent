...
| 🔗 | <a href="./docs/frontend-api-service-guide.md">前端 API Service 分层指南</a> | 说明 Axios 解包、认证 token、错误处理和新增 service 约定 |
...
| 📡 | <a href="./docs/api-response-contract.md">API 响应契约指南</a> | 说明 `Result` 响应、错误码、前端解包和认证失效处理 |
| 💬 | <a href="./docs/rag-chat-request-flow.md">RAG 问答请求链路说明</a> | 串联 SSE、会话记忆、改写、意图、检索、Prompt 和模型输出 |
| 🧾 | <a href="./docs/prompt-template-guide.md">Prompt 模板指南</a> | 理解模板路径、section、变量替换、场景选择和排查方式 |
| 🧯 | <a href="./docs/local-troubleshooting-faq.md">本地开发排查 FAQ</a> | 排查 Java、Maven、Node、数据库、中间件、模型 Key 和 SSE 问题 |
...
| 🧱 | <a href="./docs/ingestion-pipeline-extension-guide.md">入库 Pipeline 扩展指南</a> | 扩展节点接口、条件表达式、上下文传递、日志和测试 |
